...
| M4 | IN PROGRESS | Phase 3 — Payments (Stripe, pay-on-arrival model) | Stripe account live 2026-08-13, sole proprietorship. 4 Payment Links created and verified (correct pricing + $3 shipping on DIY gear). DIY gear links wired live into `/order.html`. Service links saved in `templates/stripe_payment_links.md` for manual send once a mirror's confirmed repairable. **Still needed:** one real end-to-end test transaction + refund before fully trusting it for real customers. | `site/order.html`, `templates/stripe_payment_links.md` |
